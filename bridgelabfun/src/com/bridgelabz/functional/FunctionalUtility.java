...
public static String accept(){
		...
		String name = FunctionalUtility.stringScanner();            //Enter the String to replace
		if(name.isEmpty())
		{
...
/**
 * leapyear method is used to find the particular year is leap or not
 * @param year
 * @return 
 * @return 
 */
...
public static void functionGenerator(int numberofiterations, int a[],int count) {
   ...
   public static void printWriter() throws Exception {
	...
	public static  String acceptValues() 
		...
		int row = FunctionalUtility.integerScanner();
		int column = FunctionalUtility.integerScanner();
		System.out.println("Enter the values in 2D Array :");
		a=new int[row][column];
			 String s = "";
			for (int i = 0; i < a.length; ++i) {
		        for(int j = 0; j < a[i].length; ++j) {
		    		a[i][j] = FunctionalUtility.integerScanner();
...
public static void calculateEuclideanDistance()
{
	...
	int xcoordinate  = FunctionalUtility.integerScanner();
	System.out.println("Enter the Y-Coordinate :");
	int ycoordinate = FunctionalUtility.integerScanner();
